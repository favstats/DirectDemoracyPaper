...
combined <- vdem %>% 
  ...
  mutate_at(vars(demsat, age, income, work, educ, work, polintr), as.numeric) %>% 
  mutate(polintr = 5 - polintr) %>% 
  filter(!(cntry %in% c("Switzerland", "Slovenia")))



fit1 <- lme4::lmer(demsat ~ age + income + work + educ + work + polintr + polintr*direct_dem + (1|cntry), data = combined)
texreg::screenreg(fit1)
sjPlot::plot_model(fit1, show.data = T, type = "pred",  terms = "direct_dem")

sjPlot::plot_model(fit1, type = "int")
sjPlot::sjp.int(fit1, p.kr = F, swap.pred = F)
```
