...
```{r}
pacman::p_load(tidyverse, magrittr, sjmisc, stringr)
...
sudd_dat %<>% 
  ...
  mutate(bindingness = case_when(
    ...
    str_detect(abstimmungstyp, "ad hoc") ~ "ad hoc",
    TRUE ~ NA_character_))
...
sudd_dat %>%
  ...
  filter(entity == "independent") %>%
  select(type, bindingness) %>%
  table()
...
urls <- c("http://aceproject.org/epic-en/CDTable?question=DD003&view=country&set_language=en",
  "http://aceproject.org/epic-en/CDTable?question=DD004&view=country&set_language=en",
  "http://aceproject.org/epic-en/CDTable?question=DD005&view=country&set_language=en",
  "http://aceproject.org/epic-en/CDTable?question=DD006&view=country&set_language=en",
  "http://aceproject.org/epic-en/CDTable?question=DD007&view=country&set_language=en",
  "http://aceproject.org/epic-en/CDTable?question=DD129&view=country&set_language=en",
  "http://aceproject.org/epic-en/CDTable?question=DD130&view=country&set_language=en",
  "http://aceproject.org/epic-en/CDTable?question=DD131&view=country&set_language=en")

ace_html <- read_html("http://aceproject.org/epic-en/CDTable?question=DD004&view=country&set_language=en")
var_name <- ace_html %>% 
  rvest::html_nodes("h3") %>% 
  html_text()

ace_data <- ace_html %>% 
  rvest::html_nodes("table .CDlisting") %>% 
  html_table()

ace_data <- ace_data[[1]]

names(ace_data) <- c("cntry", var_name, "comments", "verified")


# ace_dat <- ace_data %>% 
#   janitor::clean_names() %>% 
#   select(cntry, are_there_any_legal_provisions_for_mandatory_referendums_at_the_national_level)

cbind(ace_dat, ace_data %>% 
  janitor::clean_names() %>% 
  select(are_there_any_legal_provisions_for_optional_referendums_at_the_national_level))
...
```{r}
library(rvest)
navigator_html <- read_html("http://www.direct-democracy-navigator.org/countries?view=table")

navigator_dat <- navigator_html %>% 
  rvest::html_nodes("table") %>% 
  html_table()

navigator_dat <- navigator_dat[[1]]

navigator_dat %<>% filter(Name != "Saint Lucia (Saint Lucia)")

rif_navigator <- navigator_dat %>% 
  mutate(cntry = countrycode::countrycode(Name, "country.name", "country.name")) %>% 
  mutate(cntry = ifelse(is.na(cntry), "Micronesia (Federated States of)", cntry)) %>% 
  select(cntry, local, regional, national)

save(rif_navigator, file = "data/rif_navigator.Rdata")
```
